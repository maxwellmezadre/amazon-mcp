...
A Amazon.com.br entrega o histórico de pedidos cifrado no HTML. Cada card
chega como `<div class="csd-encrypted-sensitive">` mais um payload, e só o
JavaScript da própria página, com a chave no cookie `csd-key`, transforma
aquilo em DOM. Medido na conta real: um `fetch` com a sessão perfeita devolve
os contêineres e nada dentro deles.
...
e devolve o HTML já decifrado, sem `<script>`. O parsing acontece fora do
navegador, com `node-html-parser`, sobre essa string.
