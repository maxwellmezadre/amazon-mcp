...
|---|---|---|
| `auth_status` | não | Diz se há uma sessão da Amazon salva e o que ela cobre (cookies de autenticação, chave de descriptografia dos pedidos, validade, navegador de origem). |
| `login` | sim | Abre o Chrome na máquina onde este servidor roda para você entrar na Amazon à mão e salva a sessão criptografada. |
| `doctor` | não | Diagnóstico camada a camada: configuração, sessão, navegador, página de pedidos, sanidade do número do pedido, página de detalhe, fechamento das contas e cache. |
| `sync` | sim | Atualiza o cache local com os pedidos da Amazon. |
| `list_orders` | não | Lista os pedidos da Amazon a partir do cache local, do mais novo para o mais antigo, com os itens de cada um. |
| `get_order` | não | Detalhe completo de um pedido: itens com preço unitário e vendedor, todos os subtotais (frete, promoção, pontos de recompensa, imposto), forma de pagamento com parcelamento, endereço e links de nota fiscal. |
| `search_products` | não | Busca por texto em tudo que já foi comprado (título e vendedor), respondendo do cache, sem rede. |
| `spending_summary` | não | Soma quanto foi gasto na Amazon, agrupado por mês, ano, vendedor, forma de pagamento, tipo de pedido, ou aberto por composição (frete, promoção, pontos, imposto). |
| `installments_schedule` | não | Cronograma PROJETADO das parcelas em aberto, mês a mês. |
| `get_invoice` | não | Devolve os links de nota fiscal de um pedido: o resumo para impressão (sempre) e o PDF da NF-e quando o vendedor emitiu uma. |
| `download_invoice` | sim | Salva a nota fiscal de um pedido em disco, dentro de AMAZON_EXPORT_DIR. |
| `export` | sim | Exporta os pedidos ou os itens do cache para um arquivo JSON ou CSV, dentro de AMAZON_EXPORT_DIR. |
| `raw_get` | não | Abre uma página da Amazon no navegador e devolve o HTML já descriptografado, sem interpretar. |
...
| `timeout_seconds` | integer | não | Tempo para concluir o login na janela (default 300) (≥ 60, ≤ 900) |
...
| `max_requests` | integer | não | Teto de páginas carregadas nesta chamada (default 15) (≥ 1, ≤ 100) |
| `with_details` | boolean | não | Busca a página de detalhe de cada pedido (preço unitário, vendedor, parcelamento). Default true |
| `year` | string | não | Sincroniza só este período, ex.: year-2024 (`^(year-\d{4}|last30|months-3)$`) |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| `from` | string | não | Data inicial (YYYY-MM-DD) (`^\d{4}-\d{2}-\d{2}$`) |
| `to` | string | não | Data final (YYYY-MM-DD) (`^\d{4}-\d{2}-\d{2}$`) |
...
| `limit` | integer | não | Máximo de itens (default 50) (≥ 1, ≤ 200) |
| `offset` | integer | não | Itens a pular (paginação) (≥ 0) |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| `order_id` | string | sim | Número do pedido na Amazon, como aparece em `list_orders` (ex.: 702-1234567-1234567) (`^[A-Z]?\d{2,3}-\d{7}-\d{7}$`) |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| `query` | string | sim | Termo de busca (mín. 2 chars) |
| `from` | string | não | Data inicial (YYYY-MM-DD) (`^\d{4}-\d{2}-\d{2}$`) |
| `to` | string | não | Data final (YYYY-MM-DD) (`^\d{4}-\d{2}-\d{2}$`) |
| `limit` | integer | não | Máximo de itens (default 30) (≥ 1, ≤ 200) |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| `from` | string | não | Início do cronograma (YYYY-MM-DD, default hoje) (`^\d{4}-\d{2}-\d{2}$`) |
| `months` | integer | não | Quantos meses à frente (default 12) (≥ 1, ≤ 36) |
...
| `max_bytes` | integer | não | Corta o HTML neste tamanho (default 65536) (≥ 1024, ≤ 65536) |
