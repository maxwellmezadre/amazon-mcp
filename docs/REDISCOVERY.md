...
## 2. Veja a página como ela está hoje

```sh
amazon raw "/your-orders/orders?timeFilter=year-2026" --ready "body" --json > /tmp/page.json
```

`raw_get` devolve o HTML já descriptografado. Só aceita caminhos de leitura de
pedidos; qualquer outro é recusado.
...
## 3. Olhe com a janela aberta

`AMAZON_HEADLESS=0 amazon doctor` roda o mesmo diagnóstico com o Chrome visível.
É a forma de ver um desafio anti-bot ou uma tela nova que o seletor de prontidão
não reconhece.
...
O login, o segundo fator e o captcha são do usuário. Nenhuma tool submete
formulário, clica em botão de compra ou altera a conta, e `raw_get` recusa
qualquer caminho fora das páginas de pedidos. Um desafio anti-bot é sinal para
parar, não para tentar de novo.
