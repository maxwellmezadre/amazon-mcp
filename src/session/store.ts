...
// The cookie jar IS the account (HttpOnly authentication cookies included), so
// it is encrypted at rest with AES-256-GCM and written 0600. Key:
// AMAZON_SESSION_KEY wins; otherwise a random key lives next to the file (0600)
// so the tool works with zero configuration. File layout, all binary:
...
const SessionSchema = Type.Object({
  ...
  /**
   * The User-Agent of the browser that minted these cookies. Replayed verbatim
   * by the headless transport: a fingerprint that shifts between runs on the
   * same session is a classic bot signal, and headless Chrome would otherwise
   * announce itself as "HeadlessChrome".
   */
  userAgent: Type.String(),
  /** Unix ms of the last save (login or cookie rotation). */
  savedAt: Type.Number(),
});
...
export function decrypt(key: Buffer, blob: Buffer): Buffer {
  ...
  try {
    ...
    throw new SessionError(
      `Não foi possível decifrar session.enc: a chave mudou (AMAZON_SESSION_KEY ou session.key). ${LOGIN_HINT}`,
    );
  }
}
...
export function resolveKey(source: KeySource, createIfMissing: boolean): Buffer | null {
  ...
  if (existsSync(source.keyPath)) {
    ...
    if (key.length !== SESSION_KEY_BYTES) {
      throw new SessionError(
        `${source.keyPath} inválida: esperado base64 de ${SESSION_KEY_BYTES} bytes. Apague o arquivo e rode \`amazon login\`.`,
      );
    }
    return key;
  }
...
export function createSessionStore(
  ...
  return {
    ...
    load() {
      ...
      if (!key) {
        throw new SessionError(
          `${config.sessionPath} existe, mas não há chave para decifrá-lo (AMAZON_SESSION_KEY ou ${config.keyPath}). ${LOGIN_HINT}`,
        );
      }
    ...
    save(data) {
      ...
      // Write-then-rename: the MCP server may persist a cookie rotation while
      // `amazon login` writes a brand-new jar; readers never see a half-written
      // file.
