...
import { authCookieNames, hasCsdKey, type Cookie, inSiteDomain, registrableDomain, type SameSite } from "./jar.js";
...
// browser. If Amazon rotates a cookie on one side, the other may expire —
...
export function decryptCookieValue(encrypted: Buffer, key: Buffer, hostKey: string): string {
  ...
  if (version !== "v10") {
    throw new LoginError(`Formato de cookie desconhecido ("${version}"): só o formato v10 do Chromium no macOS é suportado.`);
  }
  let plain: Buffer;
  try {
...
/** Reads the cookies of `host` (and its subdomains) from a copy of the browser's database. */
export function readBrowserCookies(dbPath: string, key: Buffer, host: string): Cookie[] {
  if (!existsSync(dbPath)) {
    throw new LoginError(`Banco de cookies não encontrado em ${dbPath}. O navegador está instalado e já foi aberto alguma vez?`);
  }
  // The browser keeps the file open/locked: work on a copy (with WAL sidecars).
  const scratch = mkdtempSync(join(tmpdir(), "amazon-cookies-"));
  try {
...
export type ImportOptions = {
  browser: BrowserId;
  /** Wipe the automation browser profile first: Amazon then sees a brand-new device next to the fresh session. */
  fresh?: boolean;
...
export async function importFromBrowser(ctx: Ctx, opts: ImportOptions, deps: ImportDeps = {}): Promise<LoginResult> {
  ...
  if ((deps.platform ?? process.platform) !== "darwin") {
    throw new LoginError(
      "Importar cookies do navegador só funciona no macOS por enquanto (chave no Keychain). Use `amazon login` com o Chrome.",
    );
  }
  ...
  // The registrable domain (`amazon.com.br`), so the LIKE picks up the
  // `.amazon.com.br` cookies as well as the host-only ones of `www`.
  const host = registrableDomain(new URL(ctx.config.baseUrl).hostname);
  const cookies = readBrowserCookies(deps.dbPath ?? cookieDbPath(opts.browser), key, host);
  const auth = authCookieNames(cookies);
  if (auth.length === 0) {
    throw new LoginError(
      `O ${label} não está logado na Amazon (nenhum cookie de autenticação encontrado). ` +
        `Entre em ${ctx.config.baseUrl} no ${label} e tente de novo.`,
    );
  }
  if (!hasCsdKey(cookies)) {
    report(
      `Aviso: o ${label} não tinha o cookie csd-key. Abra "Seus pedidos" nele e importe de novo se os pedidos vierem vazios.`,
    );
  }
  ...
  report(`Sessão importada do ${label} (${cookies.length} cookies, autenticação por ${auth.join(", ")}).`);
  ...
  return {
    ...
    userAgent: userAgentFor(chromiumVersion),
