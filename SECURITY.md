...
segundo fator. O mesmo vale para `browser-profile/`, o perfil do Chrome, que
guarda a sessão logada em claro. Todo o resto deste documento decorre disso.
...
| Escrita em disco | Só `export` e `download_invoice`, e só dentro de `AMAZON_EXPORT_DIR`. Nome de arquivo é higienizado e o caminho resolvido é conferido contra o diretório |
...
| Link de nota fiscal | A URL da NF-e é pré-assinada e expira em ~3 minutos: é tratada como credencial, buscada na hora e nunca devolvida por `get_invoice` nem gravada em log |
...
Isso apaga a cópia local. Para invalidar do lado do Amazon, saia da conta
